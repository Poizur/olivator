...
function mapRetailer(row: Record<string, unknown>): Retailer {
  ...
  return {
    ...
    legalizationStatus: (row.legalization_status as Retailer['legalizationStatus']) ?? null,
    legalizationStatusAt: (row.legalization_status_at as string) ?? null,
    legalizationConsentRef: (row.legalization_consent_ref as string) ?? null,
...
export const getProductsWithOffers = cache(async (): Promise<Array<Product & { cheapestOffer: ProductOffer | null }>> => {
  ...
  for (const row of (data ?? []) as Record<string, unknown>[]) {
    ...
    const offer: ProductOffer = {
      ...
      // pagePrice/priceMismatch/manualOverride: přidáme po DB migraci (20260729)
    }
