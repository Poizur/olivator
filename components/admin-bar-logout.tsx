...
export function AdminBarLogout({ variant = 'dark' }: Props = {}) {
  ...
  const className =
    variant === 'light'
      ...
      : variant === 'dark-ghost'
        ? 'text-[12px] text-text2 hover:text-text border border-off2 hover:border-off2 rounded-md px-3 py-1.5 transition-colors'
        : 'px-2 py-0.5 hover:bg-off2 rounded transition-colors text-text2 hover:text-text'
  return (
