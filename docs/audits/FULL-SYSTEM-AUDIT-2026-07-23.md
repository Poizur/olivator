...
**Datum:** 2026-07-23 | **Verze kódu:** `b92d464` → opravy `ce3e269`+ | **Auditor:** Claude Code

> Cíl: komplexní inventura celého systému.
> **Corrigendum 2026-07-23:** Po ověření follow-up dotazy byly 4 nálezy opraveny a 3 nesprávné závěry korigovány — viz sekce `[OPRAVENO]`.
...
| F — Data integrity | ✅ price_history se píše (jen specialty shopy) | 0 | 3 |
| G — Security & GDPR | ✅ 3 nepoužívané routy smazány | 0 | 2 |
...
**A-1: `cron:newsletter-send` — manuální send by design** [OPRAVENO]
Script `scripts/cron/newsletter-send.ts` existuje. Railway service záměrně chybí — admin generuje draft středu a schvaluje ho ručně před odesláním (newsletter_auto_send = false je výchozí). Není to mezera, je to workflow.
...
**D-1: /index-cen — 639 Kč/l není v SSR HTML** → **[OPRAVENO]**
ISR cache (`revalidate=86400`) byla vytvořena před prvním snapshotem. **Oprava aplikována:** `scripts/cron/price-index.ts` nyní po uložení snapshotu volá `POST /api/admin/revalidate` (autorizace přes `x-cron-secret`), který spouští `revalidatePath('/index-cen')`. Od 1.8. nová data naskočí ihned po cron runu. Zároveň byl `/api/admin/revalidate` rozšířen pro CRON_SECRET auth.
...
| F-5 | price_history za 7 dní | **7 specialty shopů OK, 26 XML/mainstream bez záznamu** | ⚠ WARN (architektonické) |
...
**F-5: price_history — 7 specialty shopů píše, 26 XML/mainstream nepíše** ⚠ [UPŘESNĚNÍ AUDITU]

**Follow-up ověření (2026-07-23):** Původní audit byl nepřesný — označil F-5 jako KRITICKÉ s "0/26 retailerů". Skutečný stav:
- price_history celkem: **7 360 záznamů**, nejnovější dnes 04:57 UTC — tabulka funguje ✅
- 7 specialty shopů píše denně: `reckonasbavi`(249), `olivum`(244), `italyshop`(225), `greekmarket`(191), `milujemekretu`(91), `olivarna`, `reckyeshop`
- XML feed-sync retaileři (rohlik, kosik, mall, itesco, kaufland, albert, globus, iherb, mujbio, gaea, olivovyolej + discovery bez XML) **nepíší do price_history** — zapisují pouze do `product_offers` (aktuální ceny)

**Architektonický záměr vs. mezera:** Specialty shopy jsou skrapovány Playwrightem (discovery) — ten historii píše. XML retaily mají jen `product_offers`. Toto je architektonická mezera pro Price Index budoucích měsíců, ale **ne přímá závada** — data existují pro retailery co je mají.

**Dopad:** Price Index srpen 2026 padne na zálohu z `product_offers` (dostatečný, 478 produktů). Historická řada bude jen ze specialty shopů dokud feed-sync nezačne INSERT do price_history.
...
**G-1: 3 nepoužívané HTTP routy — smazány** → **[OPRAVENO 2026-07-23]**

Follow-up ověření prokázalo: `app/api/cron/newsletter-generate`, `newsletter-send`, `price-alerts` route handlery **nebyly Railway crony volány**. Railway spouští `npm run cron:*` skripty přímo (tsx), ne přes HTTP. Routy sloužily jen jako manuální "browser trigger" záložní vstup.

Grep přes `app/`, `components/`, `lib/`, `scripts/`, `emails/` nevrátil žádnou funkční závislost — pouze comment v `emails/price-alert.tsx:2`.

**Akce:** Všechny 3 adresáře smazány. Autentizační slabina (secret v URL + open bez CRON_SECRET) tím padla.
...
| In-stock nabídky bez affiliate_url | **360** (jen discovery retailers) | 0 |
...
|---|---|---|---|---|
| 1 | **H-01** | **360 in-stock nabídek bez affiliate_url** [UPŘESNĚNÍ: rohlik/kosik/mall affiliate URL mají — 360 jsou výhradně discovery specialty retailers bez affiliate programu (olivum 95, greekmarket 77, milujemekretu 40...)] — kliknutí na tyto nabídky nevygenerují příjem. Obchodní téma, ne technický bug. | 🔴 KRITICKÁ | Příjmy |
...
| ID | Oprava |
|---|---|
| C-1 | `/zebricek/nejlepsi` → `/zebricek` v `emails/welcome-d0-deals.tsx` (řádek 327) |
| G-1 | 3 nepoužívané HTTP routy smazány (`newsletter-generate`, `newsletter-send`, `price-alerts`) |
| D-1 | `price-index.ts` nyní volá revalidaci `/index-cen` po snapshotu (via `/api/admin/revalidate`) |

### 🔴 Kritické — otevřené
| ID | Popis | Sekce |
|---|---|---|
| H-01 | **360 in-stock nabídek bez affiliate_url** (rohlik/kosik/mall MAJÍ — 360 = discovery specialty retailers bez affiliate programu) | H |
| H-02 | **203 aktivních produktů bez score (42 %)** — nefigurují v žebříčcích, lower CTR | H |

### 🟡 Střední — otevřené
| ID | Popis | Sekce |
|---|---|---|
| F-5 | feed-sync nezapisuje do price_history pro XML/mainstream retailery (architektonická mezera, ne akutní) | F |
...
*Corrigendum a opravy aplikovány: 2026-07-23 po follow-up ověřování (viz sekce `[OPRAVENO]`)*
*3 nesprávné závěry korigovány: H-01 (rohlik/kosik/mall affiliate mají), F-5 (price_history se píše), A-1 (manuální send by design)*
