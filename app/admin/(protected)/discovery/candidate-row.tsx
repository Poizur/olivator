...
const STATUS_LABELS: Record<string, { label: string; color: string }> = {
  ...
  needs_review: { label: 'Ke schválení', color: 'bg-amber-50 border-terra/30 text-amber-700' },
        ...
        {(candidate.status === 'pending' || candidate.status === 'needs_review') && (
          <>
            ...
            <button
              ...
              onClick={reject}
              disabled={busy !== null}
              className="text-text3 hover:text-amber-700 px-2 py-1 text-[11px]"
              title="Zamítnout"
            >
              ✕
            </button>
        ...
        {candidate.status === 'failed' && (
          <>
            <button
              ...
              onClick={approve}
              disabled={busy !== null}
              className="bg-terra text-text rounded-full px-3 py-1 text-[11px] font-medium hover:opacity-90 disabled:opacity-40 transition-colors"
