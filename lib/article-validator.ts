...
/** Extrahuje čísla z kontextu ZA odkazem na produkt.
 *
 *  Strategie:
 *  - Score + cena: scan na celý kontext (do prvního \n\n)
 *    → jsou dostatečně distinktivní vzory (\d{2,3}\/100, \d{3,5} Kč)
 *  - Kyselost + polyfenoly: POUZE první řádek (do \n)
 *    → zabraňuje false positive z:
 *      a) názvu jiného produktu v listině ("MANAKI 0,4% 100ml")
 *      b) obecných popisů rozsahů ("kyselost 0,3–0,4 %, polyfenoly 300–400 mg/kg")
 *
 *  Tradeoff: validator nedetekuje acid/poly pokud jsou v odděleném řádku.
 *  To je záměrné — lepší žádná detekce než false positive. */
function extractNumbers(ctx: string): {
  ...
  const line = ctx.split('\n')[0]  // pouze stejný řádek jako odkaz

  const scoreM   = ctx.match(/(\d{2,3})\/100/)
  const acidM    = line.match(/kyselost\s+(\d+[,.]\d{1,2})\s*%/)
  const polyM    = line.match(/(\d{3,4})\s*mg\/kg/)
...
export async function validateArticle(articleSlug: string): Promise<ValidationResult> {
  ...
  while ((m = linkRe.exec(body)) !== null) {
    ...
    // Ořežeme na první prázdný řádek (\n\n) — zabrání přeskakování přes odstavec
    // do nesouvisejícího textu (false positive na kyselost/cenu jiného produktu).
    const rawCtx   = body.slice(afterStart, afterStart + 300)
